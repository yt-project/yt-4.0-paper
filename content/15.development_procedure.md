...
The former modify the code in place, while the latter only report errors and code smells (such as undefined variables, unused imports, bare `except` statements...) but let human developers fix them.
...
For static code analysis we rely on the [``flake8``](https://flake8.pycqa.org/en/latest/) framework (an possibly mypy for the specialized task of type checking ??)
