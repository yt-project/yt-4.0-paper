...
For instance, there may be a cell that spans $r$ from <span id="coordinate_info_r">0.25 to 0.375</span> and 
$\theta$ from <span id="coordinate_info_theta">30 to 45</span>, which is defined by the array values defined in cell <span id="coordinate_info_ij">4, 10</span>.
...
document.addEventListener("D3Loaded", function(event) { 
  d3.selectAll("g#coordinate_space_figure g")
    .on("mouseover", (event) => {
      dataset = event.target.dataset;
      d3.select("#coordinate_info_r").text(dataset["r0"] + " to " + dataset["r1"]);
      d3.select("#coordinate_info_theta").text(dataset["theta0"] + " to " + dataset["theta1"]);
