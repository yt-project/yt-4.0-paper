...
The image buffers generated are also always available through the API, so in the not-uncommon case that researchers wish to take the genrated images and perform detailed manipulations or plot layouts, they are able to do so, using the matplotlib API they may already be familiar with.
