<!-- matcnups:
  ...
  lang5  : index
  lang6  : ghosts
  lang7  : vary_order1_or0
  lang8  : vary_order2_to6
  lang9  : vary_selector
  lang10 : vary_decomp_to0
-->
...
Figure @fig:selectors shows examples of three selections within the above domain partitions.
...
index orders (Section @SS:test_order), selector sizes (Section @SS:test_size),
and partitions of the domain between files (Section @SS:test_decomp).
...
](images/bitmap/vary_order1_or0png){#fig:test_order1}
...
partitioned between files (see Section @SS:test_decomp).
...
domain and, as seen in Section @SSS:test_order1, can become costly in terms of the memory
...
size. The same test data set from Section @SS:test_order
...
As discussed in Section @SS:decomp, a bitmap index is more effective in cases where
...
Section @SS:test_order for a description). The Morton dataset is constructed in a similar way
