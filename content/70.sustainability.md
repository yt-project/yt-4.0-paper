...
As such, in the yt development process we strive to be as accommodating and welcoming as possible, while still balancing that against the very real costs that responding to bug fixes, feature requests, and providing helpful insight with issues that may be covered by the documentation or that are known bugs.
...
At the time of the first yt method paper (which, granted, is more than a decade prior to this one) yt was distributed typically in source form with its entire dependency stack to provide a uniform experience on supercomputer installations.
...
Accepting changes into the yt codebase requires careful review, and dedication of resources (real and intellectual) that may already be allocated elsewhere.
