...
The `Dataset`-specific `UnitRegistry` also lets us encode the simulation coordinate system and scaling to physical coordinates directly into the unit system. We do this via "code units".
...
Optionally, `velocity_unit`, `pressure_unit`, `temperature_unit`, `magnetic_unit`, and `density_unit` may be defined as well if the units for these fields cannot be inferred from the mass, length, and time units.
...
We have modified the definition for `set_code_units` in the `Dataset` base class.
In this new implementation, the predefined `code_mass`, `code_length`, `code_time`, and `code_velocity` symbols are adjusted to the appropriate values and `length_unit`, `time_unit`, `mass_unit`, `velocity_unit` attributes are attached to the `Dataset` instance.
