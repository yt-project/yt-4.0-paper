...
When code units are set up for a `Dataset`, `unyt_quantity` instances corresponding to these code units are also set up and added as attributes to the `Dataset`. Examples are `length_unit`, `time_unit`, `mass_unit`, `velocity_unit`, and `magnetic_unit`:
...
Optionally, `density_unit`, `pressure_unit`, `temperature_unit`, and may be defined as well if the units for these fields cannot be inferred from the mass, length, and time units.
...
unit, `h`, which defaults to unity. The `Dataset.set_units` method updates the `h` symbol to the
correct value when loading a cosmological simulation.
