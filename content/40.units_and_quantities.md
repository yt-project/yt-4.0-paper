...
If a user needs the array in a different unit system, they can quickly convert using
`convert_to_units`, which changes the units of an array in-place, or `in_units`, which returns a
copy of the array. 
...
transformations which involve physical constants, such as:
...
* Conversions between velocity $v$ and Lorentz factor $\gamma$ via $\gamma = 1/\sqrt{1-v^2/c^2}$
* Conversions between mass and Schwarzschild radius of a black hole via $R_s = 2GM/c^2$
* Conversions between mass $m$ and Compton wavelength $\lambda$ of a particle via $\lambda = h/mc$
