...
**CC: this should not be marked as "ongoing" anymore, should it?**
...
`yt` supports octree-based AMR datasets (RAMSES, Art).
...
`yt` also fully supports accessing neighboring cells.
...
**CC: remove this? Instead of starting the search from the root grid, it could be possible to start from the central oct and go upwards. This has not been implemented yet.**
Other optimizations are possible that trade computational time with memory, for example by storing the tree as a fully-threaded structure (i.e. store pointers to the 6 neighbors sharing a face with each oct).
