...
`yt` is able to interpret three types of elements: tetrahedral elements (4 faces, 4 vertices), wedge elements (5 faces, 6 vertices) and hexahedral elements (6 faces, 8 vertices).
...
From the perspective of data access, `yt` is able to select elements of unstructured meshes that overlap with given data selectors (by utilizing point-based selection on the vertices of the elements) and return subsets of the data based on that subsetting.
